 ...
 */
export function useLivePreview({ initialData }) {
    ...
    useEffect(() => {
        ...
        const unsub = subscribe({
            onInit: async (ctx) => {
                ...
                const res = await fetch(`/api/projects/${encodeURIComponent(projectUuid)}/preview/content/${encodeURIComponent(ctx.collection)}/${encodeURIComponent(ctx.entryUuid)}`, {
                    headers: { Authorization: `Bearer ${ctx.token}` },
                });
                if (!res.ok)
                    throw new Error(`Preview API returned ${res.status}`);
                return res.json();
            },
        ...
        const visCleanup = initVisualEditing({
            allowedOrigin: (() => {
                const anc = window.location.ancestorOrigins?.[0];
                if (anc)
                    return anc;
                try {
