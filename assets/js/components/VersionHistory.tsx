...
interface Version {
  uuid: string; versionNumber: number; label: string | null;
  createdAt: string; createdBy: string | null;
}

interface Props {
  projectUuid: string; collectionSlug: string; entryUuid: string;
  onRestored?: () => void;
}
    ...
    <>
      <Card>
        ...
        <CardContent className="space-y-2 max-h-[300px] overflow-y-auto">
          {versions.length === 0 && !loading && <p className="text-xs text-muted-foreground text-center py-4">Aucune version</p>}
          {versions.map(v => (
            <div key={v.uuid} className="flex items-center justify-between p-2 rounded-lg bg-muted/50 hover:bg-muted transition-colors">
              <div>
                <div className="flex items-center gap-2">
                  <Badge variant="secondary" className="text-xs font-mono">v{v.versionNumber}</Badge>
              ...
              <Button variant="ghost" size="sm" className="h-7 text-xs" disabled={restoring !== null} onClick={() => handleRestore(v.versionNumber)}>
                {restoring === v.versionNumber ? <Loader2 className="w-3 h-3 animate-spin" /> : <RotateCcw className="w-3 h-3 mr-1" />}Restaurer
              </Button>
            </div>
          ))}
        </CardContent>
        {versions.length >= 2 && (
          <div className="px-4 pb-3">
            <Button variant="outline" size="sm" className="w-full" onClick={handleDiff}>
              <GitCompare className="w-3 h-3 mr-1" />Comparer v{diffVersions.v1} ↔ v{diffVersions.v2}
            </Button>
          </div>
        )}
      </Card>
      ...
      <Dialog open={showDiff} onOpenChange={setShowDiff}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader><DialogTitle className="flex items-center gap-2"><GitCompare className="w-4 h-4" />Diff v{diff?.version1} ↔ v{diff?.version2}</DialogTitle></DialogHeader>
          <div className="space-y-2">
            ...
            {(!diff?.changes || Object.keys(diff.changes).length === 0) && <p className="text-sm text-muted-foreground text-center py-4">Aucune différence</p>}
          </div>
          <DialogFooter><Button variant="outline" onClick={() => setShowDiff(false)}>Fermer</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </>
  );
