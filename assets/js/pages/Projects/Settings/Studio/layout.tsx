    ...
    <div>
      ...
      <Tabs defaultValue="schema" className="w-full">
        <TabsList className="mb-6 flex flex-wrap">
          ...
          <TabsTrigger value="search" className="flex items-center gap-2"><Search className="w-4 h-4" />{t('studio.tab_search')}</TabsTrigger>
          <TabsTrigger value="audit" className="flex items-center gap-2"><ScrollText className="w-4 h-4" />{t('studio.tab_audit')}</TabsTrigger>
          <TabsTrigger value="graphql" className="flex items-center gap-2"><Braces className="w-4 h-4" />{t('studio.tab_graphql')}</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="graphql"><GraphQLExplorer projectUuid={project.uuid} /></TabsContent>
      </Tabs>
    </div>
  );
}
...
function GraphQLExplorer({ projectUuid }: { projectUuid: string }) {
  ...
  return (
    <div className="space-y-6">
      <div><h2 className="text-2xl font-bold tracking-tight">{t('studio.graphql.title')}</h2><p className="text-muted-foreground">{t('studio.graphql.desc')}</p></div>
      <Card>
        <CardHeader><CardTitle className="text-sm">{t('studio.graphql.endpoint')}</CardTitle></CardHeader>
        <CardContent>
          <div className="flex items-center gap-3 mb-4">
            <code className="text-sm font-mono bg-muted px-3 py-1.5 rounded-md">POST /api/projects/{projectUuid}/graphql</code>
            <Button variant="outline" size="sm" onClick={() => navigator.clipboard.writeText(`/api/projects/${projectUuid}/graphql`)}>{t('studio.graphql.copy')}</Button>
          </div>
          <div className="bg-muted rounded-lg p-6">
            <p className="text-sm font-semibold mb-3">{t('studio.graphql.example')}</p>
            <pre className="text-xs font-mono bg-background p-3 rounded-md overflow-auto">{`# Lister les collections
query { _ping }

# Obtenir une entrée
# query { articles(uuid: "xxx-xxx-xxx") { uuid title status } }

# Créer une entrée
# mutation { createArticles(input: { title: "Nouveau" content: "..." status: "draft" }) { uuid title } }`}</pre>
          </div>
          <p className="text-xs text-muted-foreground mt-4">{t('studio.graphql.hint')}</p>
        </CardContent>
      </Card>
    </div>
  );
