...
export default function SearchPage({ project, collections }: { project: Project; collections: Collection[] }) {
  ...
  return (
    <div className="space-y-6">
      <div><h2 className="text-2xl font-bold tracking-tight">{t('studio.search.title')}</h2><p className="text-muted-foreground">{t('studio.search.desc')}</p></div>
      ...
      {results && !results.error && (
        <div>
          <p className="text-sm text-muted-foreground mb-3">{t('studio.search.results', { count: String(results.hits?.length ?? results.total ?? 0) })}</p>
          <div className="space-y-3">{(results.hits || []).map((hit: any, i: number) => (
